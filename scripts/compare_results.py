...
old_result = subprocess.check_output(
    ['git', '-C', PROJECT_ROOT, 'show', 'bcb2634:samples/sample2/our_result/final_transcription.txt'], 
    text=True
)

# Get new result
with open(os.path.join(PROJECT_ROOT, 'samples/sample2/our_result/final_transcription.txt'), 'r') as f:
    new_result = f.read()

# Get ground truth
with open(os.path.join(PROJECT_ROOT, 'samples/sample2/ground_truth.txt'), 'r') as f:
    ground_truth = f.read()
