...
with open(os.path.join(PROJECT_ROOT, 'samples/sample1/our_result_previous/metadata.json')) as f:
    prev = json.load(f)

# Load new metadata  
with open(os.path.join(PROJECT_ROOT, 'samples/sample1/our_result/metadata.json')) as f:
    new = json.load(f)

# Load previous transcription
with open(os.path.join(PROJECT_ROOT, 'samples/sample1/our_result_previous/final_transcription.txt')) as f:
    prev_text = f.read()

# Load new transcription
with open(os.path.join(PROJECT_ROOT, 'samples/sample1/our_result/final_transcription.txt')) as f:
    new_text = f.read()
